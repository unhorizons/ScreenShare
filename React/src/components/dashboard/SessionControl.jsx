...
import {
    createWebSocket,
    utils,
    /* WebRTCConnection,*/ WebRTCConnectionSocket,
} from "../../utils.js";
    ...
    if (session.users) {
        viewers = session.users.map((viewer) => (
            <div key={viewer.id} className={styles.viewer}>
                ...
                {viewer._viewing && (
                    <span
                        className={`${styles.livebubble} ${styles.viewing}`}
                    ></span>
                )}
                {!viewer._viewing && (
    ...
    useEffect(() => {
        if (loading && session.slug) {
            ...
            const eventSource = new EventSource(
                `${utils.apiurl}/sessions/${session.slug}/events?token=${utils.token}`
            );
            ...
            <div className={styles.controls}>
                {session.active && (
                    <h2 className={styles.head}>
                        {" "}
                        <div
                            className={`${styles.livebubble} ${styles.active}`}
                        ></div>
                        En cours...
                    </h2>
                )}
                {!session.active && (
                    <h2 className={styles.head}>
                        {" "}
                        <div className={`${styles.livebubble}`}></div>Deconnecté
                    </h2>
                )}

                <div className={styles.btns}>
                    <button disabled onClick={endSession}>
                        Arreter
                    </button>
